...
RSpec.describe Attio::Meta do
  let(:meta_attributes) do
    {
      ...
      token: {
        ...
        name: "Test API Token",
        type: "api",
        scopes: ["record:read", "record:write", "list:read-write"]
      },
      actor: {
        type: "api_token",
        id: "token_123"
  ...
  describe "#initialize" do
    it "sets attributes correctly" do
      ...
      expect(meta.workspace).to eq(meta_attributes[:workspace])
      expect(meta.token).to eq(meta_attributes[:token])
      expect(meta.actor).to eq(meta_attributes[:actor])
    end
  end
  ...
  describe "token methods" do
    ...
    describe "#token_type" do
      it "returns the token type" do
        expect(meta.token_type).to eq("api")
      end
  ...
  describe "scope methods" do
    ...
    describe "#scopes" do
      ...
      it "returns empty array when scopes are nil" do
        meta_without_scopes = described_class.new(token: {id: "token_123"})
  ...
  describe "#to_h" do
    it "includes all meta fields" do
      meta = described_class.new(meta_attributes)
      hash = meta.send(:to_h)

      expect(hash).to eq({
        workspace: meta_attributes[:workspace],
        token: meta_attributes[:token],
        actor: meta_attributes[:actor]
      })
    end

    it "compacts nil values" do
      meta = described_class.new(workspace: {id: "ws_123"})
      hash = meta.send(:to_h)
  ...
  describe "#inspect" do
    it "includes workspace slug and token name" do
      ...
      expect(inspection).to include("token=\"Test API Token\"")
  ...
  describe "edge cases" do
    ...
    it "handles nested nil values gracefully" do
      meta = described_class.new({})
