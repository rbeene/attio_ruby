...
def process_list_entry_created(data)
  ...
  if data["list"]["name"] =~ /leads|prospects/i
...
def process_note_created(data)
  ...
  if data["content"] =~ /urgent|important|asap/i
...
get "/events" do
  ...
  if $webhook_events.empty?
    ...
    $webhook_events.reverse.first(50).each do |event|
      html += <<~HTML
        <tr>
...
get "/stats" do
  ...
  html += "<ul>"
  $webhook_events.last(10).reverse.each do |event|
