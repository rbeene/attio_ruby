  ...
  module Services
    class CompanyService < BaseService
      ...
      def add_domains(company_id, domains)
        ...
        existing_domains = Array(company[:domains]).map { |d| d[:value] }.compact
      ...
      def merge_company_data(primary, duplicate)
        # Merge domains
        primary_domains = Array(primary[:domains]).map { |d| d[:value] }.compact
        duplicate_domains = Array(duplicate[:domains]).map { |d| d[:value] }.compact
        ...
        duplicate.attributes.each do |key, value|
          next if %w[id created_at name domains].include?(key.to_s)
